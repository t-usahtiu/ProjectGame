...
public class Ovape extends Monster{
    private final BufferedImage[] image;
    private final ArrayList<Integer> direct = new ArrayList<>();

    public Ovape(int x, int y) {
        super(x, y, 2, 0, 0, 10, Sprite.score[0]);
        image = Sprite.ovape;
    }
    ...
    public void move(int nextX, int nextY) {
        ...
        while (!q.isEmpty()) {
            ...
            if (Map.getMap(tmpX + 1, tmpY) != 1 && f[tmpX + 1][tmpY] == -1) {
            ...
            if (Map.getMap(tmpX, tmpY + 1) != 1 && f[tmpX][tmpY + 1] == -1) {
